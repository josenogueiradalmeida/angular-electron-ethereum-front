module.exports.leitura = async function (msg) { 
    console.log('leitura ocorreu, ' + msg)
    await leia();
};

module.exports.escrita = async function (msg) { 
    console.log('escrita ocorreu, ' + msg)
    await escrita();
};
...
async function escrita () {
  await myContract.methods.setValue(numeroQualquer++).send({from: wallet_address, gas: 210000}, function(error, result){
    if (error) log (error);
    if (result) log ('The written is made by the transaction : ' + result);
  });  
